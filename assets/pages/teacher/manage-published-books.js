...
if ($_GET['export'] == undefined){
    url = "teacher/scripts/published-books/manage.php?staff_id="+hidden_staff_id;
}else {
    url = "teacher/scripts/published-books/manage.php?export=true&staff_id="+hidden_staff_id;
}

var publishedBooksTable;
var publishedBooksDatatable = function(){

    var handlePublishedBooksTable = function(){

        publishedBooksTable = $("#published-books-table");

        publishedBooksTable.dataTable({
            "processing": true,
            "serverSide": true,
            "ajax":{
                url: url,
        ...
        //DELETE
        publishedBooksTable.on('click', '.delete', function(e) {
            $id = $(this).attr('id');
            ...
            Swal.fire({
                ...
                if (result.value) {
                $.ajax({
                    url: "http://localhost/frcrce/teacher/scripts/published-books/delete.php",
                    method: "POST",
                    data: "delete_published_books_id=" + $id,
                    dataType: "text",
    ...
    return{
        ...
        init: function(){
            handlePublishedBooksTable();
        }
    };
}();

jQuery(document).ready(function(){
    publishedBooksDatatable.init();


    var buttons = new $.fn.dataTable.Buttons(publishedBooksTable, {
        buttons: [
            'copyHtml5',
            'excelHtml5',
            'csvHtml5',
    ...
    $('[name="published-books-table_length"]').addClass("input-sm");
});